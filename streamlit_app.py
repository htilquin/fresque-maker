    ...
    if not full_pic and len(faces) == 0:
        ...
        if full_pic:
            ...
            for face_rect in faces:
                ...
                face_image = image_clean[
                    wider_face_rect.top() : wider_face_rect.bottom(),
                    wider_face_rect.left() : wider_face_rect.right(),
                ].copy()
        ...
        st.write("⟵ Twitch the options using the sidebar :)")

        make_round = st.sidebar.checkbox("Make it round !")
        repetition = st.sidebar.slider(
            "Times the frieze is repeted in the 'frame':",
            min_value=1,
            max_value=50,
            value=5,
        )
        ...
        if images_frieze:
            if make_round:
                fresque_totale = add_frame_round(image, images_frieze, repetition)
